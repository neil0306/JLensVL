"""Fit a vision-tower Jacobian lens for the Qwen3.5-4B ViT (Phase A, vision face).

Fits the VLM *vision-encoder* Jacobian lens: for each ViT block l, estimate
J_l = E[d h_23 / d h_l] (the transport of that block's residual
into the final block's residual basis), token- and image-averaged, via backprop through the
vision tower -- reusing the exact `fit_block_jacobian` estimator (here its packed variant, as
the Qwen vision forward packs tokens with no batch axis).
...
Run (inside the `jlensvl` conda env, GPU 0 = the 96GB Blackwell):
    ...
    CUDA_VISIBLE_DEVICES=0 python examples/14_fit_vision_lens.py
